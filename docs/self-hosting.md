...
Tunnelトークンも資格情報です。保存先は `.cloudflare.env` の `TUNNEL_TOKEN` だけにし、シェル履歴、
画面共有、ログへは出さないでください。漏えい時はCloudflare側でトークンをローテーションします。
ローテーション後はCloudflareで発行された新しいトークンを `.cloudflare.env` の `TUNNEL_TOKEN` に置き換え、
次のコマンドでcloudflaredコンテナを再作成します。
